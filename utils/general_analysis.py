...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    xc = prediction[..., 4] > conf_thres  # candidates 过滤出阈值大于某一个参数的数据
    ...
    # 这是为了解决看不同层上的数据
    # print('prediction => ', prediction.shape)  # [1, 25200, 10]
    # print('labels => ', labels) # ()
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        # Apply constraints
        x = x[xc[xi]]  # confidence 置信度提取大于conf_thres的index
        # If none remain process next image
        if not x.shape[0]:
            continue
        # Compute conf
        x[:, 5:-1] *= x[:, 4:5]  # conf = obj_conf * cls_conf # 将x shape变小，将后面几个类变成最终的类
        # Box (center x, center y, width, height) to (x1, y1, x2, y2)
        box = xywh2xyxy(x[:, :4])
        conf, j = x[:, 5:-1].max(1, keepdim=True)
        source_layer = x[:, -1]
        source_layer = source_layer.unsqueeze(1)
        x = torch.cat((box, conf, j.float(), source_layer), 1)[conf.view(-1) > conf_thres]
